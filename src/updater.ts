...
autoUpdater.setFeedURL({
  url: `https://update.electronjs.org/felipap/compass/darwin-${
    process.arch
  }/${app.getVersion()}`,
})
...
async function showOutsideApplicationsFolderDialog() {
  await dialog.showMessageBox({
    type: 'info',
    message: 'Move Compass to the Applications folder',
    detail: `Compass can't be updated because it's outside the Applications folder. Move it there and try again.`,
...
async function showDownloadedDialog() {
  const result = await dialog.showMessageBox({
    ...
    detail: 'A new version of Compass is ready to be installed.',
    icon: getImagePath('original.png'),
